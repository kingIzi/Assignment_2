...
function displayFinalResult(playerScore, maxAttempts) {
...
function game() {
  ...
  while (playerScore < MAX_ATTEMPTS && computerScore < MAX_ATTEMPTS) {
    const parsedSelection = getPlayerChoice(playerScore, computerScore);
  ...
  displayFinalResult(playerScore, MAX_ATTEMPTS);
}
